<template>
  <div class="space-y-6">
    ...
    <div>
      ...
      <p class="text-xs text-slate-400 mt-1">Inventaire de votre boutique — {{ store.boutique?.nom }}</p>
    </div>
    ...
    <!-- Ajustement Modal -->
    <div v-if="selectedItem" class="fixed inset-0 z-50 bg-black/70 flex items-center justify-center p-4" @click.self="selectedItem = null">
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6 w-full max-w-sm shadow-2xl">
        <h3 class="text-base font-bold text-white mb-1">Ajuster le Stock</h3>
        <p class="text-xs text-slate-400 mb-5">{{ selectedItem.produit?.nom_commercial }}</p>
        
        <div class="flex items-center justify-center gap-4 mb-6">
          <button @click="adjustQty(-1)" class="w-10 h-10 rounded-xl bg-slate-800 hover:bg-red-900/40 text-slate-200 font-black text-xl transition-colors flex items-center justify-center">-</button>
          <span class="text-3xl font-black text-emerald-400 font-mono w-20 text-center">{{ adjustQty(0, true) }}</span>
          <button @click="adjustQty(1)" class="w-10 h-10 rounded-xl bg-slate-800 hover:bg-emerald-900/40 text-slate-200 font-black text-xl transition-colors flex items-center justify-center">+</button>
        </div>

        <div class="flex gap-3">
          <button @click="selectedItem = null" class="flex-1 py-2.5 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs rounded-xl transition-colors">Annuler</button>
          <button @click="confirmerAjustement" :disabled="saving" class="flex-1 py-2.5 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-60 text-white font-bold text-xs rounded-xl transition-colors">
