  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6">
        ...
        <p class="text-xs text-slate-400 mb-5">Rapport classique envoyé à l'administrateur.</p>
      ...
      <div class="bg-slate-900 border border-slate-800 rounded-2xl p-6">
        <h2 class="text-sm font-bold text-white uppercase tracking-wider mb-4 flex items-center gap-2">
          <BarChart3 class="w-4 h-4 text-violet-400" />
          📦 Suggestion Réappro IA
        </h2>
        <p class="text-xs text-slate-400 mb-5">Analyse IA des produits à réapprovisionner.</p>
        
        <button
          @click="fetchReappro"
          class="w-full px-5 py-3 bg-gradient-to-r from-violet-600 to-indigo-600 hover:from-violet-500 hover:to-indigo-500 text-white font-bold text-xs rounded-xl transition-all flex items-center justify-center gap-2 shadow-md shadow-violet-900/30"
        >
        ...
        <div v-if="showReappro" class="mt-5 space-y-4">
          <div v-if="reapproLoading" class="py-6 text-center text-slate-500 font-mono text-xs">
            <div class="w-5 h-5 border-2 border-violet-500 border-t-transparent rounded-full animate-spin mx-auto mb-2"></div>
            Analyse IA en cours...
          </div>
          <template v-else>
            <div class="grid grid-cols-2 gap-3">
              <div class="bg-rose-950/30 border border-rose-800/40 rounded-xl p-3">
                <div class="text-[10px] font-bold uppercase text-rose-400 mb-1">Prioritaires</div>
                <div class="text-xl font-black text-rose-300 font-mono">{{ reapproData.nombre_prioritaires }}</div>
              </div>
              <div class="bg-emerald-950/30 border border-emerald-800/40 rounded-xl p-3">
                <div class="text-[10px] font-bold uppercase text-emerald-400 mb-1">Coût estimé</div>
                <div class="text-xl font-black text-emerald-300 font-mono">{{ formatPrice(reapproData.total_estime) }}<span class="text-xs"> FCFA</span></div>
              </div>
            </div>

            <div v-if="reapproData.ai_summary" class="bg-indigo-950/50 border border-indigo-800/40 rounded-xl p-3">
              <div class="text-[10px] font-bold uppercase text-indigo-300 mb-1 flex items-center gap-1">
                <Brain class="w-3 h-3" /> Résumé IA
              </div>
              <p class="text-[11px] text-indigo-100 leading-relaxed">{{ reapproData.ai_summary }}</p>
            </div>

            <div v-if="reapproData.prioritaires.length > 0" class="border border-slate-700 rounded-xl overflow-hidden">
              <table class="w-full text-left text-[11px]">
                <thead class="bg-slate-800/80 uppercase text-[9px] text-slate-400 font-mono">
                  <tr>
                ...
                <tbody class="divide-y divide-slate-800">
                  <tr v-for="(p, i) in reapproData.prioritaires.slice(0, 8)" :key="p.id || i" class="hover:bg-slate-800/40">
                    <td class="px-3 py-2 font-semibold text-slate-200 truncate max-w-[160px]">
                      {{ p.nom_commercial || p.nom || p.produit_nom || '#' + (p.id || i) }}
                    </td>
                    <td class="px-3 py-2 text-center font-mono text-rose-400">{{ p.stock_actuel ?? p.stock_disponible ?? '?' }}</td>
                    <td class="px-3 py-2 text-center font-mono font-bold text-emerald-400">+{{ p.quantite_suggeree ?? p.quantite ?? '?' }}</td>
                  </tr>
                </tbody>
              </table>
            </div>
    ...
    <div class="bg-gradient-to-br from-slate-900 via-indigo-950/40 to-slate-900 border border-indigo-800/40 rounded-2xl p-6 shadow-2xl shadow-indigo-950/40">
      <div class="flex items-center justify-between mb-5 flex-wrap gap-3">
        <div class="flex items-center gap-3">
          <div class="w-10 h-10 rounded-xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center text-white shadow-md shadow-violet-900/40">
            <Brain class="w-5 h-5" />
          </div>
          <div>
            ...
            <p class="text-[11px] text-indigo-300/80 mt-0.5">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-5 gap-6">
        <div class="lg:col-span-2 space-y-4">
          <div class="grid grid-cols-2 gap-3">
            <div>
              <label class="block text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5">Type de rapport</label>
              <select v-model="aiRapportForm.type" class="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:border-violet-500 focus:outline-none cursor-pointer font-medium">
            ...
            <div>
              <label class="block text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5">Date</label>
              <input type="date" v-model="aiRapportForm.date" class="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:border-violet-500 focus:outline-none font-medium" />
            </div>
          </div>

          <div class="grid grid-cols-3 gap-3">
            <div>
              <label class="block text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5">CA (FCFA)</label>
              <input type="number" v-model.number="aiRapportForm.ca" class="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:border-violet-500 focus:outline-none font-mono font-semibold" />
            </div>
            <div>
              <label class="block text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5">Clients</label>
              <input type="number" v-model.number="aiRapportForm.nb_clients" class="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:border-violet-500 focus:outline-none font-mono font-semibold" />
            </div>
            <div>
              <label class="block text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5">Commandes</label>
              <input type="number" v-model.number="aiRapportForm.nb_commandes" class="w-full px-3 py-2.5 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:border-violet-500 focus:outline-none font-mono font-semibold" />
            </div>
          </div>

          <div>
            <label class="block text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5">Points clés du jour</label>
            <textarea v-model="aiRapportForm.points_cles" rows="2" placeholder="Ex: Très bonne affluence sur les engrais NPK, Vente exceptionnelle de 20 sacs d'urée..." class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:border-violet-500 focus:outline-none resize-none placeholder-slate-500"></textarea>
          </div>
          <div>
            <label class="block text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5">Anomalies / Incidents</label>
            <textarea v-model="aiRapportForm.anomalies" rows="2" placeholder="Ex: Rupture temporaire de YaraMila, Paiement Flooz en retard..." class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:border-violet-500 focus:outline-none resize-none placeholder-slate-500"></textarea>
          </div>
          <div>
            <label class="block text-[10px] font-bold uppercase text-slate-400 tracking-wider mb-1.5">État des stocks</label>
            <textarea v-model="aiRapportForm.stocks" rows="2" placeholder="Ex: Stock Urée bas (5 sacs), Bon stock sur NPK 15-15-15..." class="w-full px-3 py-2 bg-slate-800 border border-slate-700 rounded-xl text-xs text-white focus:border-violet-500 focus:outline-none resize-none placeholder-slate-500"></textarea>
          </div>

          <button
            @click="generateAiRapport"
            :disabled="aiGeneratingRapport"
            class="w-full px-5 py-3.5 bg-gradient-to-r from-violet-600 via-indigo-600 to-purple-600 hover:from-violet-500 hover:via-indigo-500 hover:to-purple-500 disabled:opacity-60 disabled:cursor-not-allowed text-white font-black text-xs rounded-xl transition-all shadow-lg shadow-violet-900/40 flex items-center justify-center gap-2 active:scale-[0.99]"
          >
        ...
        <div class="lg:col-span-3 bg-slate-950/60 border border-slate-800 rounded-2xl p-5 min-h-[480px] max-h-[600px] overflow-y-auto custom-scrollbar">
          <div v-if="aiGeneratingRapport" class="h-full flex flex-col items-center justify-center text-center py-12">
            ...
            <p class="text-sm font-bold text-white mb-1">L'IA rédige votre rapport...</p>
          ...
          <div v-else-if="!aiRapportResult" class="h-full flex flex-col items-center justify-center text-center py-12 text-slate-500">
            <div class="w-16 h-16 rounded-2xl bg-slate-800/60 border border-slate-700 flex items-center justify-center mb-4">
              <FileText class="w-8 h-8 text-slate-600" />
            </div>
            <p class="text-sm font-bold text-slate-400 mb-1">Aucun rapport généré</p>
            <p class="text-xs">Remplissez le formulaire à gauche puis cliquez sur "Générer"</p>
          </div>

          <div v-else class="space-y-4 text-[11.5px] text-slate-200">
            <div class="flex items-center justify-between flex-wrap gap-2 pb-3 border-b border-slate-800">
              <div>
                <h3 class="text-base font-black text-white mb-0.5">{{ aiRapportResult.titre || 'Rapport d\'Activité' }}</h3>
              ...
              <button
                ...
                class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white text-[11px] font-bold rounded-xl flex items-center gap-1.5 transition-colors shadow-md shadow-emerald-900/30"
              >
            ...
            <div v-if="aiRapportResult.introduction" class="space-y-1.5">
              <h4 class="text-[10px] font-black uppercase text-amber-400 tracking-wider">📌 Introduction</h4>
              <p class="text-slate-300 leading-relaxed">{{ aiRapportResult.introduction }}</p>
            </div>

            <div v-if="aiRapportResult.section_activite" class="space-y-1.5">
              <h4 class="text-[10px] font-black uppercase text-blue-400 tracking-wider">💹 Activité & Ventes</h4>
              <p class="text-slate-300 leading-relaxed whitespace-pre-line">{{ aiRapportResult.section_activite }}</p>
            </div>

            <div v-if="aiRapportResult.section_stocks" class="space-y-1.5">
              <h4 class="text-[10px] font-black uppercase text-emerald-400 tracking-wider">📦 Situation Stocks</h4>
              <p class="text-slate-300 leading-relaxed whitespace-pre-line">{{ aiRapportResult.section_stocks }}</p>
            </div>

            <div v-if="aiRapportResult.section_anomalies" class="space-y-1.5">
              <h4 class="text-[10px] font-black uppercase text-rose-400 tracking-wider flex items-center gap-1.5">
                <AlertTriangle class="w-3 h-3" />
                Anomalies Constatées
              </h4>
              <div class="bg-rose-950/30 border border-rose-900/40 rounded-xl p-3">
                <p class="text-rose-200 leading-relaxed whitespace-pre-line">{{ aiRapportResult.section_anomalies }}</p>
              </div>
            </div>

            <div v-if="aiRapportResult.section_recommandations" class="space-y-1.5">
              <h4 class="text-[10px] font-black uppercase text-violet-400 tracking-wider flex items-center gap-1.5">
                <Sparkles class="w-3 h-3" />
                Recommandations IA
              </h4>
              <div class="bg-violet-950/30 border border-violet-900/40 rounded-xl p-3">
                <p class="text-violet-200 leading-relaxed whitespace-pre-line">{{ aiRapportResult.section_recommandations }}</p>
              </div>
            </div>

            <div v-if="aiRapportResult.conclusion" class="space-y-1.5 pt-2 border-t border-slate-800">
              <h4 class="text-[10px] font-black uppercase text-indigo-400 tracking-wider">🎯 Conclusion</h4>
              <p class="text-slate-300 leading-relaxed italic">{{ aiRapportResult.conclusion }}</p>
            </div>
          </div>
        </div>
      </div>
